...
<?php
      if ($ezAdOptions['policy'] == 'unknown' && strpos($this->locale, 'en_') !== FALSE) {
?>
...
<input type="checkbox" id="ezAllowPopunder" name="ezAllowPopunder"  <?php if ($ezAdOptions['allow_popunder'] == 'Yes') { echo('checked="checked"'); }?> /> <?php _e('Allow popunders once a day to support the plugin development.', 'easy-adsenser') ; ?></label><br />
<label for="ezAllowExitJunction" title="Share your thoughts and comments on this option using the support links below.">
<input type="checkbox" id="ezAllowExitJunction" name="ezAllowExitJunction"  <?php if ($ezAdOptions['allow_exitjunction'] == 'Yes') { echo('checked="checked"'); }?> /> <?php _e('[Not implemented yet.] Allow ExitJunction to donate your search engine bounce traffic. [Not implemented yet.]', 'easy-adsenser') ; ?></label><br />
